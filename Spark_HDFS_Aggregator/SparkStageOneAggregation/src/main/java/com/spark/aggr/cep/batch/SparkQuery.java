...
public class SparkQuery extends Thread {
	...
	public SparkQuery(final SparkSession spark, final long startTime, final long endTime, final String srcpath,
			final CepHdfsFileWriter cepHdfsFileWriter) {
		this.spark = spark;
	...
	public void run() {
		try {
			...
			String query = "SELECT CONCAT(STARTTIME,'-',ENDTIME) as TimeStamp, srcaddr as src_ip, srcport as src_port,count(dOctets) as dOctetsCount, COALESCE(sum(dOctets),0) as dOctetsSum from netflowv5 where unix_secs >= STARTTIME and unix_secs < ENDTIME group by srcaddr, srcport";
